...
function createLink(heroName, text = heroName) {
  const encodedHeroName = encodeURIComponent(heroName);
  return `\x1b[4m\x1b]8;;${config.HERO_URL}/${encodedHeroName}\x1b\\${text}\x1b]8;;\x1b\\\x1b[0m`;
}
...
function createHero(hero) {
  return {
    ...
    logPos(pos) {
      console.log(
        `${createLink(this.displayName, this.displayName)} - ${(hero[`pos ${pos} winrate`] * 100).toFixed(2)}%`,
      );
    },
